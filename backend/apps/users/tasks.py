...
def send_verification_email_task(user_id: str, otp: str) -> None:
    ...
    try:
        ...
        logger.error(f"[CELERY] Attempted to send verification to non-existent user {user_id}")
...
def send_welcome_email_task(user_id: str) -> None:
    ...
    try:
        ...
        logger.error(f"[CELERY] Attempted to send welcome email to non-existent user {user_id}")
